...
def main(*args):
  ...
  flax_params = train(dir_path, classes)
  ...
  signatures = (
    save_model(predict, flax_params, model_dir, input_shape=(28, 28, 1),
               batch_sizes=[1, 256, 256], with_gradient=True))
  assert len(signatures) > 0
  signature_defs = list(signatures.keys())
  conversion_dir = os.path.join(os.path.dirname(__file__), './tfjs_models')
  convert_tf_saved_model(model_dir, conversion_dir, signature_defs[0])
